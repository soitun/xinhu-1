	...
	var c={
		showdata:function(a){
			...
			for(lx in a){
				...
				for(i=0;i<len;i++){
					s1+='<a style="TEXT-DECORATION:none" oncontextmenu="js.open(\'?a=lu&m=input&d=flow&num='+d[i].num+'\',950,500);return false;"  onclick="openinput(\''+d[i].name+'\',\''+d[i].num+'\',0)" class="list-group-item">'+d[i].name+'</a>';
				}
				s1+='</div>';
				strarr[l]+=s1;
				l++;
